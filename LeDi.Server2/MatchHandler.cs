    ...
    public class MatchHandler
    {
        ...
        private async Task UpdateMatchtimer()
        {
            ...
            if (ReferenceSecond != DateTime.Now.Second)
            {
                ...
                if (dbContext.TblMatches != null)
                {
                    ...
                    var match = dbContext.TblMatches.Single(x => x.Id == MatchId);
                    if (match.CurrentTimeLeft <= 0 &&                        
                        (match.RulePeriodOvertime == false)
                        )
                    {
                    ...
                    {
                        dbContext.TblMatches.Single(x => x.Id == MatchId).CurrentTimeLeft -= diff;
                        _logger.Debug("Setting new time left {0} (-{1}) for match {2}", dbContext.TblMatches.Single(x => x.Id == MatchId).CurrentTimeLeft, diff, MatchId);
                        await dbContext.SaveChangesAsync();
                        
                        if (DataHandler.OnMatchUpdated != null)
                            DataHandler.OnMatchUpdated(MatchId);
                    }
                }
            }
        }
        ...
        public async Task Start()
        {
            ...
            if (!IsInitialized)
            {
                ...
                if (dbContext.TblMatches != null)
                {
                    var match = dbContext.TblMatches.SingleOrDefault(x => x.Id == MatchId);
                    if (match != null)
                    {
            ...
            if (dbContext.TblMatches != null)
            {
                ...
                if (match != null)
                {
                    ...
                    if (match.MatchEvents != null)
                    {
                        ...
                        match.MatchEvents.Add(new TblMatchEvent
                        {
                            Event = matchEvent,
                            Text = matchText,
